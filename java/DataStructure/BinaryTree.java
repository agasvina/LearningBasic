...
public class BinaryTree {
    ...
    //to make it balance, we always use the middle element.
    //if the start > end return null.
    public static Node convertArray(int [] arr, int start, int end) {
        if(start > end) return null;
        int mid = (start+end)/2;
        Node root = new Node(arr[mid]);
        root.left = convertArray(arr, start, mid-1);
        root.right = convertArray(arr, mid+1, end);
        return root;
        
    }
    
    public static BinaryTree convertToBT(int [] arr){
        BinaryTree result = new BinaryTree();
        result.root =  convertArray(arr, 0, arr.length-1);
        return result;
    }
